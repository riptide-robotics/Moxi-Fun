...
public class PIDController {
    ...
    // private double pTolerance = 0.05;
    ...
    public double calculate(double currPos, double goal) {
        if (usingExternalClock) {
            throw new SecurityException("Using an externalClock!!");
        }
        if (Math.abs(currPos - goal) <= deadZone) {
            return 0;
        }
    ...
    public double calculate(double currPos, double goal, double currentTime) {
        if (!usingExternalClock) {
            ...
            throw new RuntimeException("not using an externalClock!");
        }
